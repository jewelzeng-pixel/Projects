...
# These bridges are the bridges from rows 3, 4, and 33 of the dataset.
...
def find_bridges_in_radius(bridges: list[list], lat: float, lon: float,
    ...
    for bridge in bridges:
        ...
        if bridge_id not in exclusions:
...
def get_bridge_condition(bridges: list[list], bridge_id: int) -> float:
    ...
    for bridge in bridges:
        if bridge[COLUMN_ID] == bridge_id:
            bci = bridge[COLUMN_BCI][INDEX_BCI_SCORES]
            count = 0
            while bci[count] == MISSING_BCI and count <= len(bci):
                count += 1
            if bci[count] != MISSING_BCI:
                return bci[count]
    return MISSING_BCI
...
def calculate_average_condition(bridge: list, start: int, stop: int) -> float:
    ...
    total = 0
    count = 0
    bridge_year = bridge[COLUMN_BCI][INDEX_BCI_YEARS]
    bci = bridge[COLUMN_BCI][INDEX_BCI_SCORES]
    for i in range(len(bridge_year)):
        year = int(bridge_year[i])
        score = bci[i]
...
def inspect_bridge(bridges: list[list], bridge_id: int, inspect_date: str,
    ...
    year = inspect_date[6:]
    bci_year = bridge[COLUMN_BCI][INDEX_BCI_YEARS]
    if year not in bci_year:
        bci = bridge[COLUMN_BCI][INDEX_BCI_SCORES]
        bci_year.insert(0, year)
        bci.insert(0, inspect_bci)
...
def find_worst_bci(bridges: list[list], bridge_ids: list[int]) -> int:
    """Return the bridge ID from bridge_ids of the bridge from bridges that
    ...
    1
    """
    worst_bridge_id = bridge_ids[0]
    worst_bci_score = get_bridge_condition(bridges, worst_bridge_id)
    for bridge_id in bridge_ids[1:]:
        current_bci_score = get_bridge_condition(bridges, bridge_id)
        statement = (current_bci_score == worst_bci_score and bridge_id < worst_bridge_id)
        if current_bci_score < worst_bci_score or statement:
            worst_bci_score = current_bci_score
            worst_bridge_id = bridge_id
...
def map_route(bridges: list[list], lat: float, lon: float,
    ...
    """
    visit = []
    while len(visit) < max_bridges:
        nearby_bridges = find_bridges_in_radius(bridges, lat, lon, radius, visit)
        if not nearby_bridges:
            break
        next_bridge_id = find_worst_bci(bridges, nearby_bridges)
        if next_bridge_id not in visit:
            visit.append(next_bridge_id)
            for bridge in bridges:
                if bridge[COLUMN_ID] == next_bridge_id:
...
def clean_span_data(raw_spans: str) -> list[float]:
    ...
    """
    span_lengths = []
    index_part = raw_spans.index('(')
    parts = raw_spans[index_part:]
    complete_cut = parts.split(';')
    for span in complete_cut:
        if '=' in span:
            length = span.split('=')[1]
            span_lengths.append(float(length))
    return span_lengths
...
def clean_bci_data(bci_years: list[str], start_year: int, bci_scores: list) -> \
    ...
    for i in range(len(bci_scores)):
        ...
        if years not in bci_years:
            bci_years.append(str(start_year - i))
    bci_years.sort(reverse=True)
