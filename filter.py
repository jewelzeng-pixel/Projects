...
class CustomerFilter(Filter):
    ...
    def apply(self, customers: list[Customer],
        ...
        if target_customer is None:
            return data
        customer_numbers = set(target_customer.get_phone_numbers())
        filtered_calls = []
        for call in data:
            if (call.src_number in customer_numbers
                    or call.dst_number in customer_numbers):
...
class DurationFilter(Filter):
    ...
    def apply(self, customers: list[Customer],
        ...
        Do not mutate any of the function arguments!
        """
        if len(filter_string) < 2 or not filter_string[1:].isdigit():
            return data
        ...
        filtered_calls = []
        for call in data:
            if filter_type == 'L' and call.duration < duration:
                filtered_calls.append(call)
            elif filter_type == 'G' and call.duration > duration:
                filtered_calls.append(call)
        return filtered_calls
...
class LocationFilter(Filter):
    ...
    def apply(self, customers: list[Customer],
        ...
        if len(parts) != 4:
            return data
        try:
            lowerLon, lowerLat, upperLon, upperLat = map(float, parts)
        except ValueError:
            return data
        if lowerLon > upperLon or lowerLat > upperLat:
            return data
        filtered_calls = []
        for call in data:
            for loc in [call.src_loc, call.dst_loc]:
                if (lowerLon <= loc[0] <= upperLon
                        and lowerLat <= loc[1] <= upperLat):
                    filtered_calls.append(call)
        return filtered_calls
